...
const resolvers = {
  Query: {
    users: async () => {
      return User.find().populate('message');
    },
    user: async (parent, { username }) => {
      return User.findOne({ username }).populate('message');
    },
    thoughts: async (parent, { username }) => {
      const params = username ? { username } : {};
      return Message.find(params).sort({ createdAt: -1 });
    },
    thought: async (parent, { MessageID }) => {
      return Message.findOne({ _id: MessageID });
    },
    me: async (parent, args, context) => {
      if (context.user) {
        return User.findOne({ _id: context.user._id }).populate('message');
      }
